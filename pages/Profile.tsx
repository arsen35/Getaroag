...
import { User, CreditCard, Car, Settings, PlusCircle, Edit, Trash2, LogOut, AlertTriangle, ArrowRightCircle } from 'lucide-react';
...
const ProfilePage = () => {
  ...
  const handleEditCar = (id: number) => {
    ...
    const car = myCars.find(c => c.id === id);
    if (!car) return;
    
    const newName = prompt("Araç ismini düzenle:", car.name);
    if (newName && newName.trim() !== "") {
      const updatedCars = myCars.map(c => 
  ...
  const handleDeleteCar = (id: number) => {
    if(window.confirm("Bu aracı silmek istediğinize emin misiniz? Bu işlem geri alınamaz.")) {
      const updatedCars = myCars.filter(c => c.id !== id);
      setMyCars(updatedCars);
  ...
  const handleSettings = () => {
    ...
    if(newName) {
       ...
       // In real app, update DB and localStorage
  ...
  const handleDeleteAccount = () => {
    ...
    // Condition 2: Check Active Cars (Optional, but good practice in rental apps)
    if (myCars.length > 0) {
         alert("Hesabınızı silebilmek için önce listelediğiniz araçları kaldırmalısınız.");
         return;
    }
    ...
    if (window.confirm("Hesabınızı kalıcı olarak silmek istediğinize emin misiniz? Bu işlem geri alınamaz.")) {
        ...
        alert("Hesabınız silindi. Anasayfaya yönlendiriliyorsunuz.");
        navigate('/');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-300 font-sans pb-24 md:pb-0">
      <Navbar />
              ...
              <div className="space-y-6 animate-in fade-in">
                ...
                {myCars.length === 0 ? (
                    ...
                    <div key={car.id} className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm dark:shadow-none dark:border dark:border-gray-700 border border-gray-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                        ...
                        <button onClick={() => handleEditCar(car.id)} className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-4 py-2 border border-gray-200 dark:border-gray-600 text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 rounded-lg transition-colors font-medium text-sm">
            ...
            {activeTab === 'settings' && (
                <div className="space-y-6 animate-in fade-in">
                    ...
                    <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 overflow-hidden">
                        <div className="p-4 border-b border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors cursor-pointer" onClick={handleSettings}>
